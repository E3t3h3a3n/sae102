...
int main() {
  ...
  printf("Nombres de mouvements: %d", nbMvmt);
  return EXIT_SUCCESS;
}
...
void progresser(int lesX[], int lesY[], char direction, tPlateau plateau, bool *collision, bool *pomme, int numPomme, int *nbMvmt) {
    if (lesPommesX[numPomme] > lesX[0]){ // vérifie si la pomme est à droite de la tête
      effacer(lesX[TAILLE - 1], lesY[TAILLE - 1]);
      for (int i = TAILLE - 1; i > 0; i--) {
        lesX[i] = lesX[i - 1];
        lesY[i] = lesY[i - 1];
      }
      lesX[0]++; //effectue un déplacement vers la droite
      (*nbMvmt)++;
      dessinerSerpent(lesX, lesY);
    } 
    else if (lesPommesX[numPomme] < lesX[0]) { // vérifie si la pomme est à gauche de la tête
      effacer(lesX[TAILLE - 1], lesY[TAILLE - 1]);
      for (int i = TAILLE - 1; i > 0; i--) {
        lesX[i] = lesX[i - 1];
        lesY[i] = lesY[i - 1];
      }
      lesX[0]--; // effectue un déplacement vers la gauche
      (*nbMvmt)++;
      dessinerSerpent(lesX, lesY);
    }

    if (lesPommesY[numPomme] > lesY[0]) { // vérifie si la pomme est en dessous de la tête
      effacer(lesX[TAILLE - 1], lesY[TAILLE - 1]);
      for (int i = TAILLE - 1; i > 0; i--) {
        lesX[i] = lesX[i - 1];
        lesY[i] = lesY[i - 1];
      }
      lesY[0]++; //effectue un déplacement vers le bas
      (*nbMvmt)++;
      dessinerSerpent(lesX, lesY);
    } 
    else if (lesPommesY[numPomme] < lesY[0]){ // vérifie si la pomme est au dessus de la tête
      effacer(lesX[TAILLE - 1], lesY[TAILLE - 1]);
      for (int i = TAILLE - 1; i > 0; i--) {
        lesX[i] = lesX[i - 1];
        lesY[i] = lesY[i - 1];
      }
      lesY[0]--; // effectue un déplacement vers le haut
      (*nbMvmt)++;
      dessinerSerpent(lesX, lesY);
    }
